...
# required.
pkg_source="https://github.com/drush-ops/drush/releases/download/${pkg_version}/drush.phar"
...
pkg_shasum="86465bd00dfc0df83577b1e2fe356af09e95283510c4cf636947edfb97f72a25"
...
do_install() {
  ...
  cat<<EOF > "$pkg_prefix/bin/${pkg_name}"
#!/bin/sh
$(pkg_path_for dmp1ce/php)/bin/php "$pkg_prefix/bin/$pkg_filename" "\$@"
EOF
  chmod +x "$pkg_prefix/bin/${pkg_name}"

  # here's our custom do_check()
  set -eo pipefail
  "$pkg_prefix/bin/${pkg_name}" --version 2>/dev/null | grep -q $pkg_version
}
